...
VKObjectManager::~VKObjectManager() { DestroyPendingResources(); }
...
void VKObjectManager::DestroyImageView(VkImageView image) { pendingImageViews.push_back(image); }
...
void VKObjectManager::DestroyRenderPass(VkRenderPass renderPass) { pendingRenderPasses.push_back(renderPass); }
...
void VKObjectManager::DestroyFramebuffer(VkFramebuffer frameBuffer) { pendingFramebuffers.push_back(frameBuffer); }
...
void VKObjectManager::DestroyShaderModule(VkShaderModule module) { pendingShaderModules.push_back(module); }
...
void VKObjectManager::DestroyPipeline(VkPipeline pipeline) { pendingPipelines.push_back(pipeline); }

void VKObjectManager::CreateDescriptorSetLayout(VkDescriptorSetLayoutCreateInfo& createInfo,
...
void VKObjectManager::DestroyPipelineLayout(VkPipelineLayout layout) { pendingPipelineLayout.push_back(layout); }
...
void VKObjectManager::DestroyDescriptorPool(VkDescriptorPool pool) { pendingDescriptorPools.push_back(pool); }
...
void VKObjectManager::DestroySemaphore(VkSemaphore semaphore) { pendingSemaphores.push_back(semaphore); }
...
void VKObjectManager::DestroySampler(VkSampler sampler) { pendingSamplers.push_back(sampler); }
