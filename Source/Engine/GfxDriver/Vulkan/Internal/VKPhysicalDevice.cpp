...
namespace Engine::Gfx
{
    ...
    VKPhysicalDevice VKPhysicalDevice::SelectGPUAndQueryDataForSurface(VKInstance& instance, VKSurface& surface)
    {
        for (auto &physicalDevice : GetAllPhysicalDevices(&instance, &surface))
        {
            ...
            surface.QuerySurfaceDataFromGPU(&physicalDevice);
    ...
    VkFormat VKPhysicalDevice::PickSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags featureFlags)
    {
    ...
    uint32_t VKPhysicalDevice::FindMemoryTypeIndex(VkMemoryRequirements& memRequirement, VkMemoryPropertyFlags memPropertiesFlag)
    {
        ...
        for(int i = 0; i < memProperties.memoryTypeCount; ++i)
        {
            if ((1 << i & memoryTypeBits) &&
                    (memProperties.memoryTypes[i].propertyFlags & memPropertiesFlag) )
            {
                return i;
            }
        }
