...
namespace Engine::Gfx
{
    struct RenderTargetCreateHelper
    {
        public:
            ...
            RenderTargetCreateHelper(RenderTargetDescription* rtDescription, VKAppWindow* window):
                colorAttachmentCount(rtDescription->GetColorAttachmentCount()),
                window(window),
                hasDepthStencil(rtDescription->HasDepth()),
            ...
            VkFramebufferCreateInfo MakeFrameBufferCreateInfo(VkRenderPass renderPass, VkImageView* imageViews, uint32_t imageViewCount)
            {
        ...
        private:
            ...
            void MakeRenderPassCreateInfos(const RenderTargetDescription& rtDescription, const RenderPassConfig& renderPassConfig)
            {
    ...
    const std::vector<VkClearValue>& VKRenderTarget::GetClearValues()
    {
        return clearValues;
    }
    ...
    VkRenderPass VKRenderTarget::RequestRenderPass(const RenderPassConfig& config)
    {
        ...
        {
            if (pass.config == config) return pass.renderPass;
        }
        ...
        // create a frame buffer if there isn't one. Because the render passes only differ in load store operations, so all the render passes request later should be compatible with each other
        if (framebuffer_vk == VK_NULL_HANDLE)
        {
            ...
            VkFramebufferCreateInfo frameBufferCreateInfo = helper.MakeFrameBufferCreateInfo(renderPass, attachmentImageViews.data(), attachmentImageViews.size());
    ...
    const VkExtent2D& VKRenderTarget::GetSize()
    {
        return resolution; 
    }
    ...
    {
        assert(framebuffer_vk != VK_NULL_HANDLE && "frame buffer is not created yet. Maybe you should request a render pass first");
        return framebuffer_vk;
    }

    VKImage& VKRenderTarget::GetImage(uint32_t index)
    {
