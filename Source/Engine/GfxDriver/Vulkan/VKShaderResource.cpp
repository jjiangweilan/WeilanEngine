...
std::vector<VKWritableGPUResource> VKShaderResource::GetWritableResourcesNoCache(uint32_t set, VKShaderProgram* shaderProgram, VKCommandBufferProcessor* graph)
{
    ...
    for (const auto& b : descriptorSet.bindings)
    {
        ...
        auto binding = bindings.find(bindingHandle);
...
void VKShaderResource::PushDescriptorSet(
    VkCommandBuffer cmd, uint32_t set, VKShaderProgram* shaderProgram, VKCommandBufferProcessor* graph
)
{
    auto& shaderInfo = shaderProgram->GetShaderInfo();
    std::vector<VKWritableGPUResource> writableGPUResources{};
