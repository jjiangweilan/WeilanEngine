...
bool RenderPassNode::Compile(ResourceStateTrack& stateTrack)
{
    ...
    for (auto& c : colorPortsIn)
    {
        if (c->GetResource())
        {
            ...
            a.imageView = &colorImage->GetDefaultImageView();
    ...
    if (hasDepth)
    {
        depthAttachment->imageView = &((Gfx::Image*)depthPortIn->GetResourceVal())->GetDefaultImageView(),
...
bool RenderPassNode::Execute(Gfx::CommandBuffer* cmdBuf, ResourceStateTrack& stateTrack)
{
    ...
    {
        cmdBuf->BindShaderProgram(
            drawDataOverride.shader,
    ...
    if (drawDataOverride.indexBuffer)
    {
        cmdBuf->BindIndexBuffer(
            drawDataOverride.indexBuffer->buffer,
    ...
    {
        cmdBuf->DrawIndexed(
            drawDataOverride.drawIndexed->elementCount,
    ...
    if (drawList)
    {
        for (auto& drawData : *drawList)
        {
            if (drawData.shader && !drawDataOverride.shader)
            {
                cmdBuf->BindShaderProgram(
                    drawData.shader,
                    drawData.shaderConfig ? *drawData.shaderConfig : drawData.shader->GetDefaultShaderConfig()
                );
            }
            ...
            {
                cmdBuf->DrawIndexed(
                    drawData.drawIndexed->elementCount,
...
void RenderPassNode::SetColorCount(uint32_t count)
{
    ...
    if (newCount > oldCount)
    {
        ...
        {
            colorPortsIn.push_back(AddPort(
                fmt::format("Color {}", i).c_str(),
                Port::Type::Input,
                typeid(Gfx::Image),
                false,
                [](Node* bNode, Port* other, auto type)
