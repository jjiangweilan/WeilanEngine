...
void Port::Connect(Port* other)
{
    ...
    if (iter != connected.end())
        return;
    if ((!isMultiPort && !other->connected.empty()) || GetType() == other->GetType())
        return;
    ...
    if (!input->IsMultiPort())
        ...
        input->AddResource(output->GetResource());
    ...
    if (connectionCallback)
        connectionCallback(parent, other, ConnectionType::Connect);
    if (other->connectionCallback)
        other->connectionCallback(other->parent, this, ConnectionType::Connect);
}
...
void Port::Disconnect(Port* other)
{
    ...
    if (connectionCallback)
        connectionCallback(parent, other, ConnectionType::Disconnect);
    if (other->connectionCallback)
        other->connectionCallback(other->parent, this, ConnectionType::Disconnect);
