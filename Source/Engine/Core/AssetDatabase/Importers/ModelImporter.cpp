...
UniPtr<AssetObject> ModelImporter::Load(const std::filesystem::path& root,
...
bool ModelImporter::NeedReimport(const std::filesystem::path& path, const std::filesystem::path& root, const UUID& uuid)
{
    return false;
}
...
const std::type_info& ModelImporter::GetObjectType() { return typeid(Model); }
}; // namespace Engine::Internal
