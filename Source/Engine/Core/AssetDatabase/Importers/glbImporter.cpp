...
static int MapComponentTypeBitSize(const uint32_t& componentType)
{
    if (componentType == 5120 || componentType == 5121)
        ...
        return 16;
    else if (componentType == 5125 || componentType == 5126)
        return 32;
    assert(0);
    return 0;
}
...
void glbImporter::Import(const std::filesystem::path& path,
...
void glbImportHelper::AddDataToVertexDescription(uint32_t accessorIndex, VertexAttribute<T>& attr, uint32_t typeCheck)
{
    ...
    attr.data =
        std::vector<unsigned char>(startAddr, startAddr + attr.count * (uint32_t)attr.componentCount * sizeof(T));
}

void glbImportHelper::AddDataToVertexDescription(uint32_t accessorIndex,
                                                 UntypedVertexAttribute& attr,
                                                 uint32_t typeCheck)
{
    auto& acce = accessors[accessorIndex];
...
void glbImportHelper::Load(const std::filesystem::path& path)
{
    ...
    assert(j["buffer"].contains("uri") == false &&
...
UniPtr<AssetObject> glbImporter::Load(const std::filesystem::path& root,
