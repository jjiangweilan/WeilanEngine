...
private:
    ...
    template <class U>
    struct DecayPtr<UniPtr<U>>
    {
        using Type = U;
    };
    template <class U>
    struct DecayPtr<RefPtr<U>>
    {
        using Type = U;
    };
    template <class U>
    ...
    {};
    template <class U>
    struct IsRefPtr<RefPtr<U>> : std::true_type
    {};

    template <class U>
    struct IsUniPtr : std::false_type
    {};
    template <class U>
    struct IsUniPtr<UniPtr<U>> : std::true_type
    {};
    ...
    struct IsUnorderedMap<std::unordered_map<_Key, _Value>> : std::true_type
    {};
...
protected:
    ...
    public:
        ...
        virtual void Deserialize(const std::string& nameChain,
                                 AssetSerializer& serializer,
                                 ReferenceResolver& refResolver) = 0;
    ...
    public:
        template <class... Args>
        Editable(AssetObject* parent, const std::string& name, T& val) : EditableBase(parent, name), val(val)
        {
        ...
        void Deserialize(const std::string& nameChain,
                         AssetSerializer& serializer,
    ...
    private:
        ...
        void SerializePrivate(const std::string& nameChain, AssetSerializer& serializer, U& val)
        {
            ...
            {
                if (val != nullptr)
                    // we have to do reinterpret_cast here for cases where U is forward declared. The policy of Editable
                    // enfores a RefPtr or a raw pointer is pointing to an AssetObject
            ...
            else if constexpr (std::is_base_of_v<AssetObject, typename DecayPtr<U>::Type> &&
            ...
            else if constexpr (IsUnorderedMap<U>::value)
            {
                ...
                // we key use the serializer.Write here, but at the time of writing this. Write's Vector serialization
                // support is not written yet
        ...
        template <class U>
        void DeserializePrivate(const std::string& nameChain,
                                AssetSerializer& serializer,
...
protected:
    // this function gives subclass a chance to handle member dependencies when the object is inside a deserialization
    // chain
    virtual void DeserializeInternal(const std::string& nameChain,
                                     AssetSerializer& serializer,
                                     ReferenceResolver& refResolver);
...
private:
    ...
    {
        EditableMember(const std::type_info& typeInfo,
                       UniPtr<EditableBase>&& editable,
                       void* editableVal,
                       bool isAssetObjectPtr)
            : typeInfo(typeInfo), editable(std::move(editable)), editableVal(editableVal),
              isAssetObjectPtr(isAssetObjectPtr){};
...
public:
    ...
    {
        AssetMemberIteratorValue(const std::string& name,
                                 const std::type_info& typeInfo,
                                 bool isAssetObjectPtr,
                                 void* val)
            : name(name), typeInfo(typeInfo), isAssetObjectPtr(isAssetObjectPtr), val(val)
        {}
    ...
    {
        AssetMemberIterator(EditableMembers::iterator begin) : curr(begin) {}
        ...
        {
            AssetMemberIteratorValue v(curr->first,
                                       curr->second.typeInfo,
                                       curr->second.isAssetObjectPtr,
                                       curr->second.editableVal);
            return v;
        }
...
void AssetObject::SerializeMember(const char* name, T& member)
{
    ...
    this->editableMembers.emplace(std::make_pair<std::string, EditableMember>(
        std::string(name),
        EditableMember(typeid(T),
                       std::move(m),
                       (void*)&member,
                       (IsRefPtr<T>::value || std::is_pointer<T>::value) &&
                           std::is_base_of_v<AssetObject, typename DecayPtr<T>::Type>)));
}
} // namespace Engine
