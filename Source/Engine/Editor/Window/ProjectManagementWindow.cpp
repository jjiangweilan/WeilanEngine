...
{
    ProjectManagementWindow::ProjectManagementWindow(RefPtr<EditorContext> editorContext, RefPtr<ProjectManagement> projectManagement):
    ...
    void ProjectManagementWindow::Tick()
    {
        ...
        ImGui::BeginMenuBar();
        if (ImGui::MenuItem("Create New Project")) state = WindowState::CreateNewProject;
        // if (ImGui::MenuItem("Last Project")) state = WindowState::LoadLastProject;
        ImGui::EndMenuBar();

        switch(state)
        {
            case WindowState::CreateNewProject:
                CreateNewProject();
                break;
            case WindowState::LoadLastProject:
                LoadLastProject();
                break;
        }
        ImGui::End();
