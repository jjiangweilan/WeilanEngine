...
//                 if (assetData)
//                 {
//                     // this needs to be done after importing becuase if not we don't have internal game object's name
//                     to
...
//                 else
//                 {
//                     std::unique_ptr<AssetData> ad = std::make_unique<AssetData>(std::move(newAsset), path,
//                     projectRoot); ad->SaveToDisk(projectRoot); assets.Add(std::move(ad));
//                 }
//
...
void AssetDatabase::Rename(const std::filesystem::path& oldPath, const std::filesystem::path& newPath)
{
    ...
    for (auto& d : moveAssetFiles)
    {
        ...
        assets.byPath.erase(oldPath);
        assets.byPath[newPath] = d;
...
void AssetDatabase::Remove(const std::filesystem::path& path)
{
    ...
    auto RemoveAsset = [&](const std::filesystem::path& path)
    {
        ...
        if (assetData)
        {
            ...
            auto forwradIter = std::remove_if(assets.data.begin(), assets.data.end(), [&](auto& d) { return d.get() == assetData; });
            assets.data.erase(forwradIter);
        }
    };
