...
ImRect ed::Node::GetRegionBounds(NodeRegion region) const
{
    ...
    else if (m_Type == NodeType::Group)
    {
        const float activeAreaMinimumSize = ImMax(
            ImMax(Editor->GetView().InvScale * c_GroupSelectThickness, m_GroupBorderWidth),
    ...
    if (control.ActiveNode)
    {
        ...
        else if (!isDragging && m_CurrentAction && m_CurrentAction->AsDrag())
        {
            ...
            std::stable_partition(
                m_Nodes.begin(),
                m_Nodes.end(),
                [&nodes](Node* node) { return std::find(nodes.begin(), nodes.end(), node) == nodes.end(); }
            );
...
void ed::FlowAnimationController::Flow(Link* link, FlowDirection direction)
{
    ...
    animation
        ->Flow(link, editorStyle.FlowMarkerDistance, editorStyle.FlowSpeed * speedDirection, editorStyle.FlowDuration);
}
...
ed::NavigateAction::NavigateAction(EditorContext* editor, ImGuiEx::Canvas& canvas)
    : EditorAction(editor), m_IsActive(false), m_Zoom(1), m_VisibleRect(), m_Scroll(0, 0), m_ScrollStart(0, 0),
      ...
      m_ZoomLevels(
          editor->GetConfig().CustomZoomLevels.Size > 0 ? editor->GetConfig().CustomZoomLevels.Data
                                                        : s_DefaultZoomLevels
      ),
      m_ZoomLevelCount(
          editor->GetConfig().CustomZoomLevels.Size > 0 ? editor->GetConfig().CustomZoomLevels.Size
...
ed::EditorAction::AcceptResult ed::SelectAction::Accept(const Control& control)
{
    ...
    else
    {
        Object* clickedObject =
            control.ClickedNode ? static_cast<Object*>(control.ClickedNode) : static_cast<Object*>(control.ClickedLink);
...
bool ed::SelectAction::Process(const Control& control)
{
    ...
    if (ImGui::IsMouseDragging(Editor->GetConfig().SelectButtonIndex, 0))
    {
        ...
        m_CandidateObjects
            .insert(m_CandidateObjects.end(), m_SelectedObjectsAtStart.begin(), m_SelectedObjectsAtStart.end());
