...
void JsonSerializer::Serialize(std::string_view name, const std::string& val) { j[TO_JSON_PTR(name)] = val; }
void JsonSerializer::Deserialize(std::string_view name, std::string& val) { val = j[TO_JSON_PTR(name)]; }

void JsonSerializer::Serialize(std::string_view name, const UUID& uuid) { j[TO_JSON_PTR(name)] = uuid.ToString(); }
void JsonSerializer::Deserialize(std::string_view name, UUID& uuid) { uuid = j[TO_JSON_PTR(name)]; }
