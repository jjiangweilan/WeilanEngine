...
// The main shadow generation function is WriteScreenSpaceShadow(), it will read a depth texture, and write to a shadow
// texture This code is setup to target DX12 DXC shader compiler, but has also been tested on PS5 with appropriate API
// remapping. It can compile to DX11, but requires some modifications (e.g., early-out's use of wave intrinsics is not
// supported in DX11). Note; you can customize the 'EarlyOutPixel' function to perform custom early-out logic to
...
//													// Determines overall cost, as this value controls the
// length of the shadow (in pixels).
//													// The number of texture-reads performed per-thread will be
//(SAMPLE_COUNT / WAVE_SIZE + 2) * 2.
//													// Recommended starting value is 60 (This would be 4 reads
// per thread if WAVE_SIZE is 64). A value of 64 would require 6 reads.
//
//		// Not all shadow samples are treated the same:
//		//	The bulk of samples will average together in to groups of 4, to produce a slightly smoothed result (so one
// sample cannot fully show the pixel)
//		//	However, the samples very close to the start pixel can optionally be forced to disable this averaging, so a
// single sample can fully shadow the pixel (HardShadowSamples)
//		//	Plus, a number of the last (most distant) samples can (for a small cost) apply a fade-out effect to soften a
// hash shadow cutoff (FadeOutSamples)
//
//
...
//													// This trades aliasing for grounding pixels very close to
// the shadow caster.
...
// HLSL enforces that a pixel offset in a Sample() call must be a compile time constant, which isn't always required -
// and in some cases can give a small perf boost if used.
...
struct DispatchParameters
{
    ...
    // All shadow computation is performed in non-linear depth space (not in world space), so tuned value choices will
    // depend on scene depth distribution (as determined by the Projection Matrix setup).

    float SurfaceThickness; // This is the assumed thickness of each pixel for shadow-casting, measured as a percentage
                            // of the difference in non-linear depth between the sample and FarDepthValue. Recommended
                            // starting value: 0.005 (0.5%)

    float BilinearThreshold; // Percentage threshold for determining if the difference between two depth values
                             // represents an edge, and should not perform interpolation. To tune this value, set
                             // 'DebugOutputEdgeMask' to true to visualize where edges are being detected. Recommended
                             // starting value: 0.02 (2%)
    ...
    bool IgnoreEdgePixels; // If an edge is detected, the edge pixel will not contribute to the shadow.
                           // If a very flat surface is being lit and rendered at an grazing angles, the edge detect may
                           // incorrectly detect multiple 'edge' pixels along that flat surface. In these cases, the
                           // grazing angle of the light may subsequently produce aliasing artefacts in the shadow where
                           // these incorrect edges were detected. Setting this value to true would mean that those
                           // pixels would not cast a shadow, however it can also thin out otherwise valid shadows,
                           // especially on foliage edges. Recommended starting value: false, unless typical scenes have
                           // numerous large flat surfaces, in which case true.
    ...
    bool BilinearSamplingOffsetMode; // There are two modes to compute bilinear samples for shadow depth:
                                     // true = sampling points for pixels are offset to the wavefront shared ray, shadow
                                     // depths and starting depths are the same. Can project more jagged/aliased shadow
                                     // lines in some cases. false = sampling points for pixels are not offset and start
                                     // from pixel centers. Shadow depths are biased based on depth gradient across the
                                     // current pixel bilinear sample. Has more issues in back-face / grazing areas.
                                     // Both modes have subtle visual differences, which may / may not exaggerate depth
                                     // buffer aliasing that gets projected in to the shadow. Evaluating the visual
                                     // difference between each mode is recommended, then hard-coding the mode used to
                                     // optimize the shader. Recommended starting value: false
    ...
    bool DebugOutputWaveIndex;   // Debug output to visualize layout of compute wavefronts, useful to sanity check the
                                 // Light Coordinate is being computed correctly.

    // Culling / Early out:
    float2 DepthBounds; // Depth Bounds (min, max) for the on-screen volume of the light. Typically (0,1) for
                        // directional lights. Only used when 'UseEarlyOut' is true.

    bool UseEarlyOut; // Set to true to early-out when depth values are not within [DepthBounds] - otherwise DepthBounds
                      // is unused [Optionally customize the 'EarlyOutPixel()' function to perform your own early-out
                      // logic, e.g. skipping pixels that a shadow map indicates are already fully occluded] This can
                      // dramatically reduce cost when only a small portion of the pixels need a shadow term (e.g., cull
                      // out sky pixels), however it does have some overhead (~15%) in worst-case where nothing
                      // early-outs Note; Early-out is most efficient when WAVE_SIZE matches the hardware wavefront size
                      // - otherwise cross wave communication is required.
    ...
    __init(ContactShadowParameters params, int2 waveOffset)
    {
        ...
        LightCoordinate = params.lightCoord;
        WaveOffset = waveOffset;
        FarDepthValue = params.farDepthValue;
        NearDepthValue = params.nearDepthValue;
        InvDepthTextureSize = params.invDepthTextureSize;
    ...
    float FarDepthValue;  // Set to the Depth Buffer Value for the far clip plane, as determined by renderer projection
                          // matrix setup (typically 0).
    float NearDepthValue; // Set to the Depth Buffer Value for the near clip plane, as determined by renderer projection
                          // matrix setup (typically 1).

    // Sampling data:
    float2 InvDepthTextureSize; // Inverse of the texture dimensions for 'DepthTexture' (used to convert from pixel
                                // coordinates to UVs) If 'PointBorderSampler' is an Unnormalized sampler, then this
                                // value can be hard-coded to 1. The 'USE_HALF_PIXEL_OFFSET' macro might need to be
                                // defined if sampling at exact pixel coordinates isn't precise (e.g., if odd patterns
    ...
    SamplerState PointBorderSampler; // A point sampler, with Wrap Mode set to Clamp-To-Border-Color
                                     // (D3D12_TEXTURE_ADDRESS_MODE_BORDER), and Border Color set to "FarDepthValue"
                                     // (typically zero), or some other far-depth value out of DepthBounds. If you have
                                     // issues where invalid shadows are appearing from off-screen, it is likely that
                                     // this sampler is not correctly setup
};

#if !defined(WAVE_SIZE) || !defined(SAMPLE_COUNT) || !defined(HARD_SHADOW_SAMPLES) || !defined(FADE_OUT_SAMPLES)
//  #error Before including bend_sss_gpu.h, four macros must be defined to configure the shader compile: WAVE_SIZE,
//  SAMPLE_COUNT, HARD_SHADOW_SAMPLES, and FADE_OUT_SAMPLES. See the top of this file for details.
#else

bool EarlyOutPixel(DispatchParameters inParameters, int2 pixel_xy, float depth)
{
    // OPTIONAL TODO; customize this function to return true if the pixel should early-out for custom reasons. E.g., A
    // shadow map pass already found the pixel was in shadow / backfaced, etc.
...
void ComputeWavefrontExtents(
    ...
    out float2 outDeltaXY,
    out float2 outPixelXY,
...
)
{
    ...
    // For the very inner most ring, we need to interpolate to a pixel centered UV, so the UV->pixel rounding doesn't
    // skip output pixels
    float2 end_xy = lerp(
        inParameters.LightCoordinate.xy,
        start_xy,
...
void WriteScreenSpaceShadow(DispatchParameters inParameters, int3 inGroupID, int inGroupThreadID)
{
    ...
    [unroll] for (i = 0; i < READ_COUNT; i++)
    {
        ...
        // Interpolation should only occur on the minor axis of the ray - major axis coordinates should be at pixel
        // centers
        float2 read_xy = floor(pixel_xy);
        float minor_axis = x_axis_major ? pixel_xy.y : pixel_xy.x;

        // If a pixel has been detected as an edge, then optionally (inParameters.IgnoreEdgePixels) don't include it in
        // the shadow
        const float edge_skip = 1e20; // if edge skipping is enabled, apply an extreme value/blend on edge samples to
        ...
        // HLSL enforces that a pixel offset is a compile-time constant, which isn't strictly required (and can
        // sometimes be a bit faster) So this fallback will use a manual uv offset instead
        depths.x = inParameters.DepthTexture
                       .SampleLevel(inParameters.PointBorderSampler, read_xy * inParameters.InvDepthTextureSize, 0);
        depths.y = inParameters.DepthTexture.SampleLevel(
            inParameters.PointBorderSampler,
            (read_xy + offset_xy) * inParameters.InvDepthTextureSize,
            0
        );
#else
        ...
        depths.y =
            inParameters.DepthTexture
                .SampleLevel(inParameters.PointBorderSampler, read_xy * inParameters.InvDepthTextureSize, 0, offset_xy);
#endif

        // Depth thresholds (bilinear/shadow thickness) are based on a fractional ratio of the difference between
        // sampled depth and the far clip depth
    ...
    // if (inParameters.UseEarlyOut &&
    //     (inParameters.DebugOutputWaveIndex == false && inParameters.DebugOutputThreadIndex == false &&
    //      inParameters.DebugOutputEdgeMask == false))
    // {
    ...
    //     else
    //     {
    //         // This wavefront is made up of multiple small waves, so we need to coordinate them for all to early-out
    //         // together. Doing this can make the worst case (all pixels drawn) a bit more expensive (~15%), but the
    //         // best-case (all early-out) is typically 2-3x better.
    //         LdsEarlyOut = true;

    //         GroupMemoryBarrierWithGroupSync();

    //         [branch] if (early_out == false) LdsEarlyOut = false;

    //         GroupMemoryBarrierWithGroupSync();

    //         [branch] if (LdsEarlyOut) return;
    //     }
    // }
    ...
    // All values in the LDS sample list are scaled by 1.0 / sample_distance, such that all light directions become
    // parallel. The multiply by sample_distance[0] here is to compensate for the projection divide in the data. The 1.0
    // / inParameters.SurfaceThickness is to adjust user selected thickness. So a 0.5% thickness will scale depth values
    // from [0,1] to [0,200]. The shadow window is always 1 wide. 1.0 / depth_thickness_scale[0] is because
    // SurfaceThickness is percentage of remaining depth between the sample and the far clip - not a percentage of the
    // full depth range. The min() function is to make sure the window is a minimum width when very close to the light.
    // The +direction term will bias the result so the pixel at the very center of the light is either fully lit or
    // shadowed
    float depth_scale = min(sample_distance[0] + direction, 1.0 / inParameters.SurfaceThickness) * sample_distance[0] /
                        depth_thickness_scale[0];
    ...
    [unroll] for (i = SAMPLE_COUNT - FADE_OUT_SAMPLES; i < SAMPLE_COUNT; i++)
    {
        ...
        const float fade_out =
            (float)(i + 1 - (SAMPLE_COUNT - FADE_OUT_SAMPLES)) / (float)(FADE_OUT_SAMPLES + 1) * 0.75;
    ...
    // Take the average of 4 samples, this is useful to reduces aliasing noise in the source depth, especially with long
    // shadows.
